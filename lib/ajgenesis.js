...
function loadModel(name, dirname) {
    if (!name)
        return loadModel(getModelDirectory(dirname));

    if (isPlainName(name))
        name = path.join(getModelDirectory(dirname), name + '.json');
...
function saveModel(name, model, dirname) {
    if (isPlainName(name))
        return saveModel(path.join(getModelDirectory(dirname), name + '.json'), model);
